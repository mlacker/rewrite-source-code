...
垂直拆分是按业务分库，例如电商系统中可以拆分为用户库、商品库、订单库，拆分后多个 RDS 实例可以提供并发写入的负载能力。
...
根据一定的规则，如 mod id, mod hash, hash solt, 一致性哈希，按时间维度对数据进行拆分。
...
如果业务数据量很大，但大多数时候仅对近期的热点数据频繁访问，历史数据仅用于归档和统计。可以按照时间维度进行拆分。另外针对于统计需求且实时性和准确性要求不高时可以采用物化表的方式。
