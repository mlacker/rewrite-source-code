...
- 轻量级锁自旋多少次会失败膨胀到重量级锁？
  轻量级锁失败则会膨胀为重量级锁，在重量级锁加锁时会尝试通过自旋锁的方式，JDK1.6 默认10次，1.7 则根据 JVM 收集统计信息同台调整自旋时间。
- 轻量级锁升级为重量级锁的条件？等待时间多久？
  存在其它线程占用的时候，CAS 操作失败，膨胀为重量级锁。
  CAS 失败，1.6 使用 -XX:PreBlockSpin 设置自旋锁等待次数，默认为10
  1.7 加入自适应自旋锁，由虚拟机自动调整。如果自选线程之前刚刚获得过锁，则虚拟机会认为这次自旋也很有可能会成功，进而延长自旋时间。
